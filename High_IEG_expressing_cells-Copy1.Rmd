...
This dataset is sn prfiling with time course of stress data: https://www.biorxiv.org/content/10.1101/2021.03.26.436970v1.full

snRNA-seq data here: https://github.com/ETHZ-INS/StressomeExplorer/tree/main/data/snRNAseq

```{r}
# paths

jeagerpath <- "/home/acampbell/PavLabEngrams/EngramCellClassifier/Jeager2018_GSE98679/"
ayhanpath <- '/home/acampbell/test_datasets/Ayhan2021_GSE160189/'
```
...
ayhan_neurons <- ayhan_counts[,ayhan_neuron_cellids]

ayhan_neurons.meta <- ayhan_meta[ayhan_meta$Cluster %in% hippocampal_neuron_types,]
```
...
ayhan_neurons <- pavlab.normalize(ayhan_neurons, median.scale = TRUE)
...
```{r}
# broken this cell type proportions code is retunring an empty table
# the inputs are fine
...
```{r}
#ayhan_neurons.meta$iegsum_activity <- ieg_expression_sum(ayhan_neurons, ieg_list = iegs_hg)
ayhan_neurons.meta$iegsum_activity <- colSums(ayhan_neurons[iegs_hg,])
```


```{r}
median(ayhan_neurons.meta$iegsum_activity)
```


```{r}
p <- ayhan_neurons.meta %>% ggplot(aes(x=Cluster, y=iegsum_activity)) + 
  geom_boxplot() +
  geom_point()+
  xlab('Cell Type')+
  ylab('Ayhan Normalized IEG Expression Sum')+
  theme_classic()
p
```

```{r}
dg.idx <- ayhan_neurons.meta$Cluster %in% c("Den.Gyr1","Den.Gyr2")
df <- ayhan_neurons.meta[dg.idx,]
df$seurat_clusters <- as.factor(df$seurat_clusters)
p <- df %>% ggplot(aes(x=seurat_clusters, y=iegsum_activity)) + 
  geom_boxplot() +
  geom_point()+
  xlab('DGC Subtype')+
  ylab('Ayhan Normalized IEG Expression Sum')+
...
From this tutrial [here](https://satijalab.org/seurat/articles/spatial_vignette.html) in the section for SLdie-seq
...
Saudaers hippocampus cells as a seurat object.

```{r}
ref <- readRDS("/space/scratch/Saunders2018_Hp_GSE116470/mouse_hippocampus_reference.rds")
```
...
Might be best to just pull the DGC's out, we can calculate the prcomp while we're at it and save
the PC1 data.
...
```{r}
saunders2018_dgc_meta <- data.frame(celltype = ref$celltype[saunders2018.dgc.idx],
                                    cluster = ref$cluster[saunders2018.dgc.idx],
                                    subcluster = ref$subcluster[saunders2018.dgc.idx]
                                    )

saunders2018_dgc_meta$cluster <- as.factor(as.character(saunders2018_dgc_meta$cluster))
saunders2018_dgc_meta$subcluster <- as.factor(as.character(saunders2018_dgc_meta$subcluster))

ref@meta.data$subcluster[ref@meta.data$cluster=='4']


saunders2018_dgc_umi <- data.frame(as.matrix(zeigler2022.obj@assays@data$counts))
saunders2018_dgc_umi
```
...
df <- data.frame('Penk' = as.numeric(saunders2018_dgc_counts['PENK',]),
                  'Cck' = as.numeric(saunders2018_dgc_counts['CCK',]),
                 'Malat1' = as.numeric(saunders2018_dgc_counts['MALAT1',]) )
df <- pavlab.normalize(df)

df$DGC_Subtype = as.factor(as.character(saunders2018_dgc_meta$subcluster))
rownames(df) <- rownames(saunders2018_dgc_meta)

mu <- ddply(df, "DGC_Subtype", summarise, grp.mean=mean(Cck))

p<-ggplot(df, aes(x=Cck, color=DGC_Subtype)) +
  geom_density()+
  geom_vline(data=mu, aes(xintercept=grp.mean, color=DGC_Subtype),
             linetype="dashed")
p
...
SpatialFeaturePlot(slide.seq, features = c("Dentate Principal cells"), alpha = c(0.1, 1))
```
...
SpatialFeaturePlot(slide.seq, features = c("4-1", "4-2"), alpha = c(0.1, 1))
```

The detate granule cells are just very poorly defined.  Suggesting the clustering is not actually pulled out DGCs.
Since I want to sub-cluster may be worth redoing it.

```{r}
saunders2018.dgc.idx <- ref$celltype=="Dentate Principal cells"


saunders2018_dgc_counts <- data.frame(as.matrix(ref@assays$RNA@counts[,saunders2018.dgc.idx]))

saunders2018_dgc_meta <- ref@meta.data[saunders2018.dgc.idx,]


saunders2018dgcs <- CreateSeuratObject(counts = saunders2018_dgc_counts,
                                       meta.data = saunders2018_dgc_meta)
```


```{r}
saunders2018dgcs <- FindVariableFeatures(saunders2018dgcs, selection.method = "vst", nfeatures = 2000)
all.genes <- rownames(saunders2018dgcs)
saunders2018dgcs <- ScaleData(saunders2018dgcs, features = all.genes)
saunders2018dgcs <- NormalizeData(saunders2018dgcs, normalization.method = "LogNormalize", scale.factor = 10000)
saunders2018dgcs <- RunPCA(saunders2018dgcs)
```
...
# computation time
saunders2018dgcs <- JackStraw(saunders2018dgcs, num.replicate = 100)
saunders2018dgcs <- ScoreJackStraw(saunders2018dgcs, dims = 1:20)
JackStrawPlot(saunders2018dgcs, dims = 1:20)
```

```{r}
ElbowPlot(saunders2018dgcs)
```


PC15 seems to be the final useful and around 10 there is an elbow.




```{r}
saunders2018dgcs <- RunUMAP(saunders2018dgcs, dims = 1:10)
saunders2018dgcs <- FindNeighbors(saunders2018dgcs, dims = 1:10)
saunders2018dgcs <- FindClusters(saunders2018dgcs, resolution = 0.3, verbose = FALSE)
```

```{r}
DimPlot(saunders2018dgcs, reduction = "umap")
```

```{r}
saunders2018dgcs.markers <- FindAllMarkers(saunders2018dgcs, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)


saunders2018dgcs.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
```

```{r}
FeaturePlot(saunders2018dgcs, features = c("PENK","CCK"))
```

```{r}
FeaturePlot(saunders2018dgcs, features = "CCK")
```
...
## IEG expression summing

First we will see how the IEG score in the time course data in the Jeager data behaves..

```{r}
# load the jeager data
...
jeagercounts1.path <- paste(jeagerpath,'GSE98679_count.txt.gz', sep ='')
jeagercountv2.path <-paste(jeagerpath,                     'GSE98679_v2_GSM3308862-GSM3309413_count.txt.gz', sep ='')
jeagermeta.path <- paste(jeagerpath,'SraRunTable.txt', sep ='')
jeagerarclabels.path <- paste(jeagerpath, 'Jaeger2018_meta_arclabels.csv', sep ='')

jeager2018_counts <- bind_cols(read.table(jeagercounts1.path, header = TRUE, check.names = FALSE),read.table(jeagercountv2.path, header = TRUE, check.names = FALSE))
jeager2018_counts[is.na(jeager2018_counts)] <- 0

jeager2018_meta <- read.csv(jeagermeta.path, header = TRUE)
...
arclabels <- read.csv(jeagerarclabels.path, header = TRUE)
...
jeager.normed <- pavlab.normalize(jeager2018_counts, UMI = jeager2018_meta$umi)

jeager2018_meta$iegsum_activity <- ieg_expression_sum(jeager.normed, ieg_list = target_IEGs)
```



```{r}
jeager2018_meta$exposure <- factor(jeager2018_meta$exposure, 
                                   levels = c('HC','1hr','4hr','5hr','A>A','A>C'))   
```


```{r}
p<- jeager2018_meta %>% ggplot(aes(x=exposure, y=iegsum_activity)) + 
  geom_boxplot() +
  geom_point()+
  xlab('Time Point')+
  ylab('Normalized IEG Expression Sum')+
  theme_classic()
p
```

Some what surprisingly the activity appears to grow from 1hr then die down at 5hrs.  Reactivation condition does display much greater activity compared to a completely novel
exposure indicating there may be more activity.

```{r}
jpeg("ActivitySumJeagerbyTimepoint.jpg", width = 700, height = 700)
p
dev.off()
```

```{r}
saundersmetacells <- read.csv('/space/scratch/Saunders2018_Hp_GSE116470/GSE116470_metacells.BrainCellAtlas_Saunders_version_2018.04.01.csv')


saudershp_counts <- read.table('/space/scratch/Saunders2018_Hp_GSE116470/GSE116470_F_GRCm38.81.P60Hippocampus.raw.dge.txt.gz', sep = '\t')
