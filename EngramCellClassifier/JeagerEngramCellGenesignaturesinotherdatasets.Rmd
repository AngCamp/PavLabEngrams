...
We can check this by plotting the gene counts and the mitochondria RNA, it is clear they have undergone some quality control

```{r}
#make a seurat object of the adult granule cells
full.hoch5k.adultDG.data <-CreateSeuratObject(counts = hochgerner5k_2018_counts[, hoch5k.GC_Adult.p35.idx])
full.hoch5k.adultDG.data[["percent.mt"]] <- PercentageFeatureSet(full.hoch5k.adultDG.data, pattern = "^MT-")
```


```{r}
VlnPlot(full.hoch5k.adultDG.data, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
```


```{r}
plot1 <- FeatureScatter(full.hoch5k.adultDG.data, feature1 = "nCount_RNA", feature2 = "percent.mt")
plot2 <- FeatureScatter(full.hoch5k.adultDG.data, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1 + plot2
```

```{r}
# Let's try this with the whole dataset, not jsut the day 35 DG adult cells
full.hoch5k <-CreateSeuratObject(counts = hochgerner5k_2018_counts)
full.hoch5k[["percent.mt"]] <- PercentageFeatureSet(full.hoch5k, pattern = "^MT-")
```
```{r}
#looking at various QC metrics
VlnPlot(full.hoch5k, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
```
```{r}
plot1 <- FeatureScatter(full.hoch5k, feature1 = "nCount_RNA", feature2 = "percent.mt") + ggtitle("RNA count vs Percentage of Mitochondrial Genes") + theme_classic() + geom_hline(yintercept = 0) + xlab("RNA read count") + ylab("Mitochondria Gene Expression (% per cell)") + theme(plot.title = element_text(hjust = 0.5))
plot2 <- FeatureScatter(full.hoch5k, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1 #+ plot2
```

```{r}
#check the gene filtering
...
full.hoch5k.adultDG.data <- subset(full.hoch5k.adultDG.data,
                                   subset = nFeature_RNA > 20 & nFeature_RNA < 250 & percent.mt < 5)
```



<br><br>
So in the tutorial it also says 0 variable features so I guess that's OK.
<br><br>
```{r}
