...
```{r}
FeaturePlot(Hoch5k.GCadult, features = c("Penk", "Malat1"), blend =TRUE)
```


```{r}
FeaturePlot(Hoch5k.GCadult, features = c("Cck", "Malat1"), blend =TRUE)
```

```{r}
seurat_clusters.3dims
seurat_clusters.7dims
```

### Run it again with 7 dims
...
```{r}
DimHeatmap(Hoch5k.GCadult, reduction = "ica")
...
```
It may make sense to remove these JeagerDEG genes from the list entirely and try to see how it affects the clustering and cluster just using the DEGs.  Most Jeager DEGs were upregulated but some were downregulated so it may be useful to check their loading sign as well.
...
```{r}
VizDimLoadings(Hoch5k.GCadult, dims = 4, reduction = "pca")
```

```{r}
VizDimLoadings(Hoch5k.GCadult, dims = 5, reduction = "pca")
```


```{r}
VizDimLoadings(Hoch5k.GCadult, dims = 6, reduction = "pca")
```
...
```{r}
#Clustering, we will visualize in UMP later
Hoch5k.GCadult.onlyJeagerDEGs <- FindNeighbors(Hoch5k.GCadult.onlyJeagerDEGs, dims = 1:3)
...
```{r}
FeaturePlot(Hoch5k.GCadult.onlyJeagerDEGs, reduction = 'pca', 
            features = c( "Rgs4", "Rps14"), blend = TRUE)
...
I am not entriely willing to just give up on an unsupervised method for doing this.  May be with the right embedding and better data one could find a better method.
...
cluster0.markers <- FindMarkers(Hoch5k.GCadult.onlyJeagerDEGs, ident.1 = 0, min.pct = 0.25)
head(cluster0.markers, n = 10)
```
```{r}
cluster1.markers <- FindMarkers(Hoch5k.GCadult.onlyJeagerDEGs, ident.1 = 1, min.pct = 0.25)
head(cluster1.markers, n = 10)
```

```{r}
cluster2.markers <- FindMarkers(Hoch5k.GCadult.onlyJeagerDEGs, ident.1 = 2, min.pct = 0.25)
head(cluster2.markers, n = 10)
```

```{r}
cluster3.markers <- FindMarkers(Hoch5k.GCadult.onlyJeagerDEGs, ident.1 = 3, min.pct = 0.25)
...
```
```{r}
summary(abs(cluster0.markers$avg_log2FC[cluster0.markers$p_val_adj<0.05]))
summary(abs(cluster1.markers$avg_log2FC[cluster1.markers$p_val_adj<0.05]))
summary(abs(cluster2.markers$avg_log2FC[cluster2.markers$p_val_adj<0.05]))
summary(abs(cluster3.markers$avg_log2FC[cluster3.markers$p_val_adj<0.05]))
```



```{r}
sd(abs(cluster0.markers$avg_log2FC[cluster0.markers$p_val_adj<0.05]))
sd(abs(cluster1.markers$avg_log2FC[cluster1.markers$p_val_adj<0.05]))
sd(abs(cluster2.markers$avg_log2FC[cluster2.markers$p_val_adj<0.05]))
sd(abs(cluster3.markers$avg_log2FC[cluster3.markers$p_val_adj<0.05]))
```
```{r}
rownames(cluster3.markers)[cluster3.markers$p_val_adj<0.05]
