...
combined.counts <- full_join(chen2020_counts, kwon2021_counts, by = "GENE") %>%
  column_to_rownames( var = "GENE") 
...
combined.meta$og_cluster <-  c(chen2020_meta$ChenLayerMarkers, kwon2021_meta$mpfc_integrated_marker)
...
combined.meta$CellID <- colnames(combined.counts)
...
mpfc_neurons_markers <- FindAllMarkers(mpfc_neurons.combined.sct, test.use = "negbinom", only.pos = TRUE)
...
DimPlot(mpfc_neurons.combined.sct, reduction = "tsne",shape.by = "label_status", label = TRUE, pt.size = 1)
```
```{r}
# with labels
...
```{r}
FeaturePlot(mpfc_neurons.combined.sct, features = "Nrgn",  label = TRUE)
```

```{r}
FeaturePlot(mpfc_neurons.combined.sct, features = "Serinc3", split.by = "label_status", label = TRUE)
```
...
```{r}
gaba_or_glut <- c("Excitatory", "Excitatory","Excitatory","Excitatory","Inhibitory","Inhibitory","Excitatory")
...
mpfc_neurons.combined.sct@meta.data$CellID

combined.meta.thisrun <- mpfc_neurons.combined.sct@meta.data
write_csv(combined.meta.thisrun,"~/PavLabEngrams/EngramCellClassifier/RemoteMemory/chen_kwon_combined_meta.csv")
```
...
chen_normed <- chen_normed[,!(names(chen_normed) %in% "TdTom-transgene")] # remove engram label expression
...
classifier.Excitatory_mpfc <-  resampled.randomForest.crossvalidated( data= glut_chen_normed,
...
saveRDS(classifier.Excitatory_mpfc, "~/EngramCellModels/Excitatory_mpfc_remotememory_rf.RDS")
```
...
head(importance.df.mpfc_excitatory, 10) # don't forget to run this before gettng  the regularized
```
...
predictions <- as.numeric(predict(classifier.Excitatory_mpfc, 
                                  glut_chen_normed[,c(1:(dim(glut_chen_normed)[2]-1))],
                                  type="response"))
...
kwon2021_meta$umi <- as.numeric(colSums(kwon2021_counts[,c(1:(dim(kwon2021_counts)[2]) )])) # no minus one because there's no gene column
...
kwon_normed <- pavlab.normalize(kwon2021_counts, UMI = kwon2021_meta$umi)
kwon_normed <- scale.and.rotate(kwon_normed)
...
#jpeg("Histograme_RemoteActivatyScore.jpg", width = 350, height = 350)
