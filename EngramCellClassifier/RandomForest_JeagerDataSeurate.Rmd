...
lastcol.to.firstcol <- function(df.in){
  df.out <- df.in[,c(dim(df.in)[2], 2:(dim(df.in)[2]))]
  return(df.out)
}
...
# we must change DG.idx and not.ptz slightly to deal with the generow shifting the index
combined.counts <- jeager2018_counts[,as.numeric(c(1,DG.idx+1))] %>% 
  left_join(lacar2016_wc_counts[, c(1, not.ptz[not.ptz <= 82]+1) ]) %>%
...
rownames(combined.counts) <- combined.counts[,1]
combined.counts[,1] <- NULL

#cleaning up the gene column from the other count data
jeager2018_counts[,1] <- NULL
lacar2016_snHC_counts[,1] <- NULL
lacar2016_snNE_counts[,1] <- NULL
lacar2016_wc_counts[,1] <- NULL
